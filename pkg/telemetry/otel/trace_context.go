...
const (
	...
	traceParentHeaderAlt = "Traceparent"
	tracestateHeader     = "tracestate"
	traceStateHeaderAlt  = "Tracestate"
	delimiter            = "-"
)
...
var (
	_           propagation.TextMapPropagator = TraceContext{}
...
func (tc TraceContext) extract(carrier propagation.TextMapCarrier) trace.SpanContext {
	...
	if h == "" {
		h = carrier.Get(traceParentHeaderAlt)
		if h == "" {
	...
	// specification.
	th := carrier.Get(tracestateHeader)
	if th == "" {
		th = carrier.Get(traceStateHeaderAlt)
	}
	scc.TraceState, _ = trace.ParseTraceState(th)
	scc.Remote = true
