...
func ServeBuf(ctx context.Context, path string, rpcHandler http.Handler, port uint64, serviceName string) {
	...
	log.GlobalSugared().Infof("Buf %s listening on port %d\n", serviceName, port)
	srv := &http.Server{
	...
	go func() {
		if err := srv.ListenAndServe(); err != nil {
			if err != http.ErrServerClosed {
				...
				log.GlobalSugared().Infof("Buf %s server closed", serviceName)
			}
		}
	}()
	<-ctx.Done()
	log.GlobalSugared().Infof("Buf %s shutting down", serviceName)
