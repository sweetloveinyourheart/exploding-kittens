...
// Not safe for concurrent use by multiple goroutines without additional
// locking.
func (c *Cache[K, V]) set(key K, value V, version int64, ttl time.Duration) *Item[K, V] {
	if ttl == DefaultTTL {
		ttl = c.options.ttl
	}
...
func (c *Cache[K, V]) Set(key K, value V, ttl time.Duration) *Item[K, V] {
	...
	return c.set(key, value, -1, ttl)
}

// SetV creates a new item from the provided key and value, adds
// it to the cache and then returns it. If an item associated with the
// provided key already exists, the new item overwrites the existing one.
// NoTTL constant or -1 can be used to indicate that the item should never
// expire.
...
// the default/global TTL that was specified when the cache instance was
// created.
func (c *Cache[K, V]) SetV(key K, value V, version int64, ttl time.Duration) *Item[K, V] {
	c.items.mu.Lock()
	defer c.items.mu.Unlock()

	return c.set(key, value, version, ttl)
}

// CompareAndSet creates a new item from the provided key and value, adds
// it to the cache and then returns it. If an item associated with the
// provided key already exists, the new item overwrites the existing one.
// NoTTL constant or -1 can be used to indicate that the item should never
// expire.
...
// the default/global TTL that was specified when the cache instance was
// created.
func (c *Cache[K, V]) CompareAndSet(key K, value V, version int64, ttl time.Duration) *Item[K, V] {
	c.items.mu.Lock()
	defer c.items.mu.Unlock()
...
func (c *Cache[K, V]) GetOrSet(key K, value V, opts ...Option[K, V]) (*Item[K, V], bool) {
	...
	item := c.set(key, value, -1, setOpts.ttl)

	return item, false
}

// GetOrSetV retrieves an item from the cache by the provided key.
// If the item is not found, it is created with the provided options and
// then returned.
// The bool return value is true if the item was found, false if created
// during the execution of the method.
// If the loader is non-nil (i.e., used as an option or specified when
// creating the cache instance), its execution is skipped.
func (c *Cache[K, V]) GetOrSetV(key K, value V, version int64, opts ...Option[K, V]) (*Item[K, V], bool) {
	c.items.mu.Lock()
	defer c.items.mu.Unlock()

	elem := c.getWithOpts(key, false, opts...)
	if elem != nil && (version == -1 || elem.version > version) {
		return elem, true
	}
	...
	item := c.set(key, value, version, setOpts.ttl)
...
func (c *Cache[K, V]) Start() {
	...
	timer := timeutil.Clock.Timer(waitDur())
	stop := func() {
		if !timer.Stop() {
