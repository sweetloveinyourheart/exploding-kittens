...
func Test_Cache_set(t *testing.T) {
	...
	for cn, c := range cc {
		...
		t.Run(cn, func(t *testing.T) {
			...
			item := cache.set(c.Key, "value123", -1, c.TTL)
...
func Test_Cache_CompareAndSet(t *testing.T) {
	cache := prepCache(time.Hour, "test1", "test2", "test3")
	item := cache.SetV("test1", "value123", 5, time.Minute)
	require.NotNil(t, item)
	assert.Equal(t, "value123", item.Value())
	assert.EqualValues(t, 5, item.Version())

	item = cache.CompareAndSet("test1", "value124", 4, time.Minute)
	require.NotNil(t, item)
	assert.Equal(t, "value123", item.Value())
	assert.EqualValues(t, 5, item.Version())

	item = cache.CompareAndSet("test1", "value125", 6, time.Minute)
	require.NotNil(t, item)
	assert.Equal(t, "value125", item.Value())
	assert.EqualValues(t, 6, item.Version())
}
