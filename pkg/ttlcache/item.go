...
func (item *Item[K, V]) update(value V, version int64, ttl time.Duration) {
	item.mu.Lock()
...
func (item *Item[K, V]) touchUnsafe() {
	...
	item.expiresAt = timeutil.Clock.Now().Add(item.ttl)
}
...
func (item *Item[K, V]) isExpiredUnsafe() bool {
	...
	return item.expiresAt.Before(timeutil.Clock.Now())
}
