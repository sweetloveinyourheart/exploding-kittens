...
func CreateNATSRepoHand(ctx context.Context, appID string, mw ...eventing.EventHandlerMiddleware) (eventing.ReadRepo[Hand, *Hand], error) {
	...
	var domainRepo eventing.ReadWriteRepo[Hand, *Hand]
	domainRepo, err = natsRepo.NewRepo[Hand, *Hand](ctx,
		constants.HandStream,
		SubjectFactory,
		entityProjector,
	...
	domainProjector = projector.NewEventHandler[Hand, *Hand](entityProjector, domainRepo)
...
func AddNATSHandCommandHandlers(ctx context.Context, appID string, commandBus *bus.CommandHandler, mw ...eventing.CommandHandlerMiddleware) error {
	connPool, err := do.InvokeNamed[*pool.ConnPool](nil, string(constants.ConnectionPool))
	if err != nil {
		return err
	}
