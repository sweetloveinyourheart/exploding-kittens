...
var (
	...
	BatchSize                       = 1024
	DeskStateHandlerType            = "desk-stateprocessing"
...
func (w *DeskStateProcessor) HandleDeskShuffled(ctx context.Context, event common.Event, data *desk.DeskShuffled) error {
	log.Global().DebugContext(ctx, "desk shuffled", zap.String("desk_id", data.GetDeskID().String()), zap.String("game_id", data.GetGameID().String()), zap.String("player_id", data.GetPlayerID().String()))
