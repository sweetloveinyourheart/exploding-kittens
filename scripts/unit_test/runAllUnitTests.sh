...
function runTest() {
    if [ $verbose -eq 1 ]; then
        $1 && ( kittens-echo-green "$1 - PASS" && exit 0 ) || ( kittens-echo-red "$1 - FAIL" && exit 1 )
    else
        export NO_CONSOLE_COLORING=true
        $1 &> tests/logs/log-$2 && ( kittens-echo-green "$1 - PASS" && exit 0 ) || ( kittens-echo-red "$1 - FAIL" && exit 1 )
    fi
}
...
if [ $exitCode -eq 0 ]; then
    kittens-echo-green "All tests passed."
else
    kittens-echo-red "Some tests failed.  Run the individual make command(s) or review the logs for more details."
fi
